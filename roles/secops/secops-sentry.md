...
*   **Verification**: Ensure all database tables in `public` schemas have RLS enabled:
    ...
    ```
*   **Tenant Isolation**: Ensure all query selection, insertion, and update policies strictly filter by the authenticated user's tenant context to prevent cross-tenant data leaks:
    ```sql
